...
const image=effective==="dark"?`${base}assets/menu/air-tahiti-night-background.jpg?v=8`:`${base}assets/menu/air-tahiti-home.jpg?v=11`;
document.documentElement.style.setProperty("--att-bg-image",`url("${new URL(image,location.href).href}")`);
}
...
body::before{background-image:var(--att-bg-image)!important}
body.theme-light::before{background-color:#eaf5f8!important}
body.theme-dark::before{background-color:#07111f!important}
...
if(media.addEventListener)media.addEventListener("change",handle);else if(media.addListener)media.addListener("change",handle);
...
e.preventDefault();document.body.classList.remove("page-enter-active");document.body.classList.add("page-exit");setTimeout(()=>{window.location.href=href},280);
});
